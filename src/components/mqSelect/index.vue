...
<script lang="ts" setup>
   ...
   */
  import { withDefaults, reactive, computed, watch } from 'vue';
  ...
  const loadList = async () => {
    if (
      !props.getApi ||
      (data.page.total && data.dataList.length >= data.page.total)
    )
      return;
    ...
    try {
      ...
      if (
        data.dataList.length >= data.page.total &&
  ...
  const onVisibleChange = (isShow: boolean) => {
    if (props.getApi) {
      ...
      if (
        isShow &&
        isSelect.length === 0 &&
        Object.keys(data.remoteParams).length !== 0
      ) {
        data.remoteParams = {};
        onReset();
      }
